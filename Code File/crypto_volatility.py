...
# ===============================
# Predict next-day volatility as previous day's volatility
y_persistence = y_true_vol[:-1]
y_true_persist = y_true_vol[1:]

mse_persist = mean_squared_error(y_true_persist, y_persistence)
rmse_persist = np.sqrt(mse_persist)
mae_persist = mean_absolute_error(y_true_persist, y_persistence)

print("\nPERSISTENCE BASELINE")
print(f"MSE : {mse_persist:.6f}")
print(f"RMSE: {rmse_persist:.6f}")
print(f"MAE : {mae_persist:.6f}")

# ===============================
# MARKET-ONLY LSTM (NO SENTIMENT)
# ===============================
...
X_market = df[features_market_only].values
X_market = scaler_X.fit_transform(X_market)

mse_market, rmse_market, mae_market = train_and_evaluate(
    X_market, y, "MARKET-ONLY LSTM (NO SENTIMENT)"
)
...
X_no_lag = df[features_no_lag].values
X_no_lag = scaler_X.fit_transform(X_no_lag)

_, rmse_no_lag, _ = train_and_evaluate(
    X_no_lag, y, "ABLATION – NO LAG FEATURES"
)
